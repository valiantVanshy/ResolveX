 ...
 */
function showTranslationIndicator(show = true, message = '🔄 Translating website...') {
    let indicator = document.getElementById('translationIndicator');

    if (show && !indicator) {
        indicator = document.createElement('div');
        indicator.id = 'translationIndicator';
...
/**
 * Get all elements that contain text to translate
 */
function getTranslatableElements() {
    const textSelectors = [
        'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
        'p', 'span:not(.notranslate)', 'div:not(.notranslate)',
        'label', 'button:not(#languageBtn):not(.close-btn)',
        'td', 'th', 'li', 'a',
        '.form-label', '.stat-label', '.metric-label',
        '.admin-subtitle', '.login-subtitle'
    ];
    ...
    textSelectors.forEach(selector => {
        try {
            document.querySelectorAll(selector).forEach(element => {
                // Skip if element has children with text (to avoid duplicating translations)
                const hasTextChildren = Array.from(element.children).some(child => 
                    child.textContent && child.textContent.trim()
                );

                // Only add if element has direct text content and not many child elements
    ...
    // Handle input and textarea placeholders separately
    ...
    document.querySelectorAll('option').forEach(element => {
        if (element.textContent && element.textContent.trim() && element.value !== '') {
            elements.push(element);
        }
    });
...
function generateElementKey(element) {
    ...
    return actualElement.tagName + '_' + 
           (actualElement.id || 'no-id') + '_' + 
           (actualElement.className || 'no-class') + '_' + 
           (actualElement.textContent || actualElement.placeholder || '').substring(0, 20);
}
...
function storeOriginalTexts() {
    ...
    console.log('Storing original texts...');

    const elements = getTranslatableElements();

    elements.forEach(item => {
        const key = generateElementKey(item);

        if (item.type === 'placeholder') {
            originalTexts.set(key, item.element.placeholder);
        } else {
...
/**
 * Translate the entire website to the target language
 */
async function translateWebsite(targetLanguage) {
    ...
    isTranslating = true;
    showTranslationIndicator(true, '🔄 Preparing translation...');
    ...
    try {
        ...
        const elements = getTranslatableElements();
        console.log(`Translating ${elements.length} elements to ${targetLanguage}...`);

        let translated = 0;
        const total = elements.length;
        const batchSize = 3; // Translate 3 elements at a time

        for (let i = 0; i < elements.length; i += batchSize) {
            const batch = elements.slice(i, i + batchSize);

            // Update progress
            showTranslationIndicator(true, `🔄 Translating... ${translated}/${total} complete`);
            ...
            const promises = batch.map(async (item) => {
                try {
                    ...
                    if (item.type === 'placeholder') {
                        originalText = originalTexts.get(key);
                        if (originalText) {
        ...
        showTranslationIndicator(true, '✅ Translation complete!');
        setTimeout(() => showTranslationIndicator(false), 2000);

        console.log(`Translation to ${targetLanguage} completed successfully!`);

    } catch (error) {
        console.error('Website translation error:', error);
        showTranslationIndicator(true, '❌ Translation failed!');
        setTimeout(() => showTranslationIndicator(false), 3000);
...
function restoreOriginalText() {
    ...
    elements.forEach(item => {
        ...
        if (item.type === 'placeholder') {
            ...
            if (originalText) {
                item.element.placeholder = originalText;
            }
        } else {
            const element = item.element || item;
...
/**
 * Main function to change language - called when user clicks language option
 */
async function changeLanguage(langCode, langDisplayName) {
    console.log(`Changing language to: ${langCode} (${langDisplayName})`);
    ...
    if (currentLangSpan) {
        currentLangSpan.textContent = langDisplayName;
    }
    ...
    await translateWebsite(languageCodes[langCode]);

    // Save language preference
    localStorage.setItem('preferredLanguage', langCode);
    localStorage.setItem('preferredLanguageDisplay', langDisplayName);
}
...
function initializeTranslationSystem() {
    ...
    // Wait for page to fully load, then store original texts
    setTimeout(() => {
        storeOriginalTexts();

        // Load saved language preference
        const savedLang = localStorage.getItem('preferredLanguage');
        const savedLangDisplay = localStorage.getItem('preferredLanguageDisplay');

        if (savedLang && savedLang !== 'en' && languageCodes[savedLang]) {
            console.log(`Loading saved language: ${savedLang}`);
            changeLanguage(savedLang, savedLangDisplay || languageNames[savedLang]);
        }
    }, 2000); // Wait 2 seconds for all content to load
}
